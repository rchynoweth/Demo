...
# MAGIC To run this demo, just select the cluster `dbdemos-mlops-end2end-ryan_chynoweth` from the dropdown menu ([open cluster configuration](https://adb-984752964297111.11.azuredatabricks.net/#setting/clusters/1101-160109-7fp4jd2a/configuration)). <br />
...
import mlflow
model_name = f"{catalog}.{db}.advanced_mlops_churn_rac"

xp_path = f"/Users/ryan.chynoweth@databricks.com/"

print(f"Finding best run from {xp_name} and pushing new model version to {model_name}")

mlflow.set_experiment(f"/Users/ryan.chynoweth@databricks.com/{xp_name}")
...
print(f"Registering model to {catalog}.{db}.advanced_mlops_churn_rac")

# Get the run id from the best model
run_id = best_model.iloc[0]['run_id']

# Register best model from experiments run to MLflow model registry
model_details = mlflow.register_model(f"runs:/{run_id}/model", f"{catalog}.{db}.advanced_mlops_churn_rac")
...
client.set_registered_model_alias(
  name=f"{catalog}.{db}.advanced_mlops_churn_rac",
  alias="Challenger",
  version=model_details.version
)
